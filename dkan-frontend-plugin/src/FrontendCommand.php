...
class FrontendCommand extends BaseCommand
{
  ...
  protected function configure()
  {
    $this
      ...
      ->setDescription('Set frontend repo.')
      ->setHelp(
        <<<EOT
...
//    $output->writeln('FOUND: ' . $dkan_package->getName());
    ...
    if ($dkan_extra = $dkan_package->getExtra() ?? FALSE) {
      $dkan_frontend = $dkan_extra['dkan-frontend'] ?? static::$defaultFrontendRepo;
    }
    $output->writeln(print_r($dkan_frontend, true));

    //    $repos = $this->getComposer()->getPackage()->getRepositories();
//    $output->writeln(print_r($repos, true));
    ...
    $repo = [
      'type' => 'vcs',
      'url' => $dkan_frontend['url'],
    ];
    $this->jsonFile = new JsonFile(Factory::getComposerFile(), null, $this->getIO());
    $config_source = new JsonConfigSource($this->jsonFile);
    $config_source->addRepository(static::$pluginPrefix . hash('crc32', $repo['url']), $repo, false);

//    $config_source->addLink();
