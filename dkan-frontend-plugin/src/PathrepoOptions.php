...
class PathrepoOptions {
   ...
   *
   * @param array $options
   *   The options taken from the 'pathrepo-plugin' section.
  ...
  protected function __construct(array $options, array $scaffoldOptions = []) {
    ...
    if ($value = $scaffoldOptions['locations']['project-root'] ?? FALSE) {
      if ($project_root !== FALSE && $project_root !== $value) {
        throw new \Exception('pathrepo plugin configuration locations:project-root can not override Drupal scaffold plugin.');
      }
      $project_root = $value;
    }
    if ($value = $scaffoldOptions['locations']['web-root'] ?? FALSE) {
      if ($web_root !== FALSE && $web_root !== $value) {
        throw new \Exception('pathrepo plugin configuration locations:web-root can not override Drupal scaffold plugin.');
      }
      $web_root = $value;
    }
  ...
  public function notSymlinkedMessage() {
    ...
    return [
      "The pathrepo plugin never performed the symlinks.",
